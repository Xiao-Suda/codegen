...
pub fn add_stdlib(text: &mut TextSeg, data: &mut DataSeg) {
	...
	// fn rand(max: int): int
	let mut f = Function::new_global("rand");
		f.inst(Lw(A1, 0, Sp));   // lw a1, 0(sp)
		f.inst(Li(A0, 0));       // li a0, 0
		f.inst(Li(V0, 42));      // li v0, 42
		f.inst(Syscall);         // syscall
		f.inst(Addi(Sp, Sp, 4)); // addi sp, sp, 4
		f.inst(Jr(Ra));          // jr ra
	text.add(f);
	...
	// fn rt$streq(a: string, b: string): bool
	let mut f = Function::new_global("rt$streq");
		f.inst(Lw(T3, 0, Sp)); // t3 = a
		f.inst(Lw(T4, 4, Sp)); // t4 = b

		f.inst(Li(V0, 0)); // set return value to 0 (false)

		f.label("_loop");
			f.inst(Lb(T0, 0, T3)); // t0 = *a
			f.inst(Lb(T1, 0, T4)); // t1 = *b

			f.comment("if either character is zero, let the condition after the loop decide.");
			f.inst(Beq(T0, Zero, "_break".into()));
			f.inst(Beq(T1, Zero, "_break".into()));

			f.comment("if the characters are unequal, the strings are unequal, so return false.");
			f.inst(Bne(T0, T1, "_return".into()));

			f.comment("characters are equal here, so increment pointers and check next character");
			f.inst(Addi(T3, T3, 1));
			f.inst(Addi(T4, T4, 1));
		f.inst(J("_loop".into()));
		...
		f.comment("if either character loaded is nonzero, the strings are unequal.");
		f.comment("if they're both zero, this sum will be zero, meaning they're the same.");
		f.inst(Add(T0, T0, T1));
		f.inst(Bne(T0, Zero, "_return".into()));
