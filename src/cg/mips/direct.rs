...
const TEMP_REGS: &[Reg] = &[
	...
	// technically, we could use t0-t9 as well, and treat them as just 10 more "saved" regs,
	// giving us a new max depth of 18 registers. but cmon. this is just a toy. real register
	// allocation is the right solution.
];

/// how many temporary registers are allowed, which complex expressions
/// may be able to exceed (causing a compiler crash lol).
...
impl From<usize> for LdStSize {
	fn from(s: usize) -> LdStSize {
		match s {
			1 => return LdStSize::Byte,
			4 => return LdStSize::Word,
			_ => panic!(),
		}
	}
}
...
impl StructLayout {
	...
	fn get_field(&self, field_sym_id: SymbolId, reg: Reg) -> VarLoc {
		...
		return VarLoc::Field(reg, offs, size);
	}
}
...
impl RegAllocator {
	...
	fn max_regs(&self) -> usize {
		return self.max_regs;
	}

	fn cur_reg(&self) -> Reg {
		return TEMP_REGS[self.cur_reg];
	}
...
impl CgFuncBuilder {
	...
	fn new_dummy(frame: StackFrame) -> Self {
		return Self {
			name: "<dummy>".into(),
			code: vec![],
			frame,
	...
	fn finish(self, name_prefix: Option<&str>, ctx: &CgCtx) -> CgFunction {
		// 1. finish the stack frame by telling it how many saved regs we need.
		let mut frame = self.frame;
		frame.add_saved_regs(self.regs.max_regs());

		// 2. come up with the name and whether or not it's global.
		...
		// 3. put some helpful comments in the output to show the stack frame layout.
		...
		// 4. actually output the prologue, code, and epilogue.
		add_prologue(&mut func, &frame);
		func.extend(&self.code);
		add_epilogue(&mut func, &frame, is_main);

		// 5. done!
		return CgFunction { func, frame };
	}

	fn finish_just_code(self) -> CgFunction {
		let mut func = Function::new("<dummy>");
		func.extend(&self.code);
		return CgFunction { func, frame: self.frame };
	}

	fn get_sym_var_loc(&self, sym_id: SymbolId) -> Option<VarLoc> {
		return self.frame.get_sym_var_loc(sym_id);
	}
	...
	fn cur_reg(&self) -> Reg {
		return self.regs.cur_reg();
	}

	fn push_reg(&mut self) -> Reg {
		return self.regs.push_reg();
	}

	fn pop_reg(&mut self) {
		return self.regs.pop_reg();
	}
	...
	fn move_(&mut self, dst: Reg, src: Reg) {
		// the tiniest, itty-bittiest optimization
		if src != dst {
...
impl<'ctx> DirectCg<'ctx> {
	...
	pub(crate) fn run_no_stdlib(prog: &Program, nc: &'ctx NameCtx, tc: &'ctx TypeCtx)
	-> (TextSeg, DataSeg) {
		let mut cg = Self::new(CgCtx::new(nc, tc));
		cg.visit_program(prog);
		return cg.finish_no_stdlib();
	}

	pub(crate) fn run_stmt(stmt: &Box<Stmt>, nc: &'ctx NameCtx, tc: &'ctx TypeCtx)
	-> (TextSeg, DataSeg) {
		let mut cg = Self::new(CgCtx::new(nc, tc));
		let mut frame = StackFrameBuilder::new();

		StackFrameVisitor(|vd| {
			let sym_id = cg.ctx.sym_declared_by(&vd.name);
			let size = cg.get_load_store_size(sym_id);
			frame.add_var(sym_id, size);
		}).visit_stmt(stmt);

		let mut b = CgFuncBuilder::new_dummy(frame.finish());
		cg.visit_stmt(stmt, &mut b);
		cg.funcs.push(b.finish_just_code());
		return cg.finish_no_stdlib();
	}

	pub(crate) fn run_exp(exp: &Box<Exp>, nc: &'ctx NameCtx, tc: &'ctx TypeCtx)
	-> (TextSeg, DataSeg) {
		let mut cg = Self::new(CgCtx::new(nc, tc));
		let mut b = CgFuncBuilder::new_dummy(StackFrameBuilder::new().finish());
		cg.visit_exp(exp, &mut b);
		cg.funcs.push(b.finish_just_code());
		return cg.finish_no_stdlib();
	}
	...
	fn finish_no_stdlib(self) -> (TextSeg, DataSeg) {
		// self.dump();

		return (self.build_text_seg(), self.data);
	}
	...
	fn get_sym_var_loc(&self, id: &Ident, b: &CgFuncBuilder) -> VarLoc {
		...
		match sym.kind {
			LocalVar =>
				return b.get_sym_var_loc(sym_id)
					.expect("local variable was not added to stack frame"),

			GlobalVar | ExternVar => {
				let size = self.get_load_store_size(sym_id);
				return VarLoc::Global(sym.name.clone(), size);
			}

			Function | ExternFunc =>
				return VarLoc::Func(sym.name.clone()),
	...
	fn get_field_var_loc(&self, obj: &Box<Exp>, name: &Ident, reg: Reg) -> VarLoc {
		...
		return self.get_struct_layout(struct_sym_id).get_field(field_sym_id, reg);
	}

	#[allow(dead_code)]
	fn get_struct_method_name(&self, obj: &Box<Exp>, name: &str) -> String {
		let struct_name = self.ctx.sym_name(self.get_struct_sym_id(obj));
		return make_method_name(struct_name, name);
	}

	#[allow(dead_code)]
	fn get_load_store_size(&self, sym_id: SymbolId) -> LdStSize {
		match self.ctx.sym_type(sym_id).as_ref() {
			Type::Bool => return LdStSize::Byte,
			_          => return LdStSize::Word,
		}
	}
	...
	fn get_struct_sym_id(&self, e: &Box<Exp>) -> SymbolId {
		return self.ctx.node_type(e.id).symbol_id();
	}

	#[allow(dead_code)]
	fn visit_args(&mut self, this: Option<&Box<Exp>>, args: &Vec<Box<Exp>>, b: &mut CgFuncBuilder) {
	...
	/// generate a function call to the `rt$concat` runtime function
	#[allow(dead_code)]
	...
	/// generate a function call to the `rt$streq` runtime function
	#[allow(dead_code)]
	fn call_rt_streq(&mut self, dst: Reg, lhs: &Box<Exp>, rhs: &Box<Exp>, b: &mut CgFuncBuilder) {
		b.make_arg_space(2);
		let reg = self.visit_exp(lhs, b);
		b.set_arg(0, reg);
		let reg = self.visit_exp(rhs, b);
		...
		match &e.kind {
			...
			Call { callee, args } => {
				match &callee.kind {
					...
					Field { obj, name } =>
						self.method_call(&obj, &name.name, args, b),
					_ =>
						self.indirect_call(callee, args, b),
				}
			...
			Parens(exp) => {
				self.visit_exp_into_reg(exp, dst, b);
			}
		}
	}
