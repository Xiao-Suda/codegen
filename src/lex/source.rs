...
impl Source {
	...
	pub fn from_file(filename: &str) -> std::io::Result<Self> {
		...
		return Ok(Self::from_str(&code, filename));
	}
	...
	pub fn iter(&self) -> SourceIter<'_> {
		return SourceIter::new(&self.filename, &self.code);
	}
	...
	pub fn location_from_offset(&self, offs: usize) -> Location {
		...
		return Location::new(&self.filename, line, col);
	}
}
...
impl<'src> SourceIter<'src> {
	...
	pub fn loc(&self) -> Location {
		return Location::new(self.filename, self.line, self.col);
	}
	...
	pub fn cur(&self) -> char {
		return *self.code.get(self.pos).unwrap_or(&'\0');
	}

	/// Gets the character at the *next* position, or `'\0'` if the next position is
	/// past the end of the source.
	pub fn peek(&self) -> char {
		return *self.code.get(self.pos + 1).unwrap_or(&'\0');
	}
...
impl Location {
	...
	pub(crate) fn new(filename: &str, line: usize, col: usize) -> Self {
		return Self { filename: filename.into(), line, col };
	}
