...
impl LexError {
	...
	pub fn invalid_char(loc: Location) -> LexError {
		return LexError { loc, kind: LexErrorKind::InvalidChar };
	}

	pub fn unclosed_string(loc: Location) -> LexError {
		return LexError { loc, kind: LexErrorKind::UnclosedString };
	}

	pub fn unknown_escape(loc: Location) -> LexError {
		return LexError { loc, kind: LexErrorKind::UnknownEscape };
	}

	pub fn int_range(loc: Location) -> LexError {
		return LexError { loc, kind: LexErrorKind::IntOutOfRange };
	}

	pub fn incomplete_int(loc: Location) -> LexError {
		return LexError { loc, kind: LexErrorKind::IncompleteInt };
	}
}
