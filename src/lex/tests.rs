...
fn complex_symbols() {
	...
	test_ok("< <= <== > >= >==", vec![
			Less, LessEq, LessEq, Assign,
			Greater, GreaterEq, GreaterEq, Assign, Eof]);
	test_err("!", LexErrorKind::InvalidChar, (1, 1));
}
...
fn keywords() {
	...
	test_ok("return and or not true false struct new null",
		vec![Return, And, Or, Not, True, False, Struct, New, Null, Eof]);
	...
	test_ok(r##"
		...
		Fn, id("main"), LParen, RParen, LBrace,
			...
			If, id("x"), LessEq, IntLit(0xFEED, Base::Hex), LBrace,
				id("println_s"), LParen, id("y"), RParen, Semi,
			RBrace,
			RBrace, Eof
		]
	);
